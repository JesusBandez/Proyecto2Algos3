...
public class CFC(val g: GrafoDirigido) {
  ...
  */
   ...
   private fun dfsVisit(g: Grafo, u: Int) {
        ...
        // Agregar a componente conexa y guardar su identificador
        cfc[arboli].add(u)
        propiedadesDeVertice[u].identificadorCFC = arboli
     ...
     */
      ...
      return propiedadesDeVertice[v].identificadorCFC   
    }
    ...
    fun obtenerGrafoComponente() : GrafoDirigido {
      ...
      */
      ...
      // Crear matriz booleana para llevar los arcos que se han agregado de componente a componente
      var ladoYaEnGrafoComponente = List(this.numeroDeCFC(), {MutableList(this.numeroDeCFC(), {false})})
      ...
      for (verticeInicial in 0 until g.obtenerNumeroDeVertices()){
        for (arco in g.adyacentes(verticeInicial)){
          ...
          // Si las componentes son distintas y no se ha agregado el arco, se agrega el arco.
          if ( (componenteInicial !=  componenteFinal) && !ladoYaEnGrafoComponente[componenteInicial][componenteFinal]){
            
            //agregar arco
            grafoComponente.agregarArco(Arco(componenteInicial,componenteFinal)) 

            // Marcar que ya existe un lado entre estas componentes
            ladoYaEnGrafoComponente[componenteInicial][componenteFinal] = true           
          }
        }
      }
    ...
    identificadorCFC: Muestra el indice de la lista de componentes donde se encuentra el vertice
    */   
    inner class Vertice(
